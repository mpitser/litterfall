...
for r in cur.fetchall():
	...
	d.execute("SELECT d.trap, d.type, d.value as mass from jlm_litterfall_data as d where d.sample_id = %d" % r["row_id"])
	
	if d.rowcount==0:
		# Try alternate table
		d.execute("SELECT d.trap, d.type, d.value as mass from jlm_litterfall_data3 as d where d.sample_id = '%s'" % r["sample_id"])
